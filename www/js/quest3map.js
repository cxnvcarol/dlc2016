...
{
    //leafletMap3.remove();
    //leafletMap3 = L.map('map').fitBounds(mapBounds);
    /*L.tileLayer('http://{s}.tiles.wmflabs.org/bw-mapnik/{z}/{x}/{y}.png', {
     maxZoom: 18,
     attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
     }).addTo(leafletMap3);*/
...
{
    leafletMap3.remove();
    leafletMap3 = L.map('quest3map').fitBounds(mapBounds);
    L.tileLayer('http://{s}.tiles.wmflabs.org/bw-mapnik/{z}/{x}/{y}.png', {
        maxZoom: 18,
        attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
    }).addTo(leafletMap3);
...
function drawOnMap(lat, long, name, color)
{
    ...
    leafletMap3.addLayer(aPointOnTheMap);
    aPointOnTheMap.bindPopup(name);
}

function removePoint(pointInstance)
{
    leafletMap3.removeLayer(pointInstance);
}
...
{
    triInstance.removeLayer(leafletMap3);
}
...
function drawMarkerOnMap(lat, lng, name, icon)
{
    ...
    aPointOnTheMap =  new L.marker([lat, lng]).addTo(leafletMap3);
...
leafletMap3.on('click', function(a) {console.log("lat, long: " + a.latlng.lat + "," + a.latlng.lng);});
