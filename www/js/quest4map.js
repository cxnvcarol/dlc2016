...
{
    //leafletMap4.remove();
    //leafletMap4 = L.map('map').fitBounds(mapBounds);
    /*L.tileLayer('http://{s}.tiles.wmflabs.org/bw-mapnik/{z}/{x}/{y}.png', {
     maxZoom: 18,
     attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
     }).addTo(leafletMap4);*/
...
{
    leafletMap4.remove();
    leafletMap4 = L.map('quest4map').fitBounds(mapBounds);
    L.tileLayer('http://{s}.tiles.wmflabs.org/bw-mapnik/{z}/{x}/{y}.png', {
        maxZoom: 18,
        attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'
    }).addTo(leafletMap4);
...
function drawOnMap(lat, long, name, color)
{
    ...
    leafletMap4.addLayer(aPointOnTheMap);
    aPointOnTheMap.bindPopup(name);
}

function removePoint(pointInstance)
{
    leafletMap4.removeLayer(pointInstance);
}
...
{
    triInstance.removeLayer(leafletMap4);
}
...
function drawMarkerOnMap(lat, lng, name, icon)
{
    ...
    aPointOnTheMap =  new L.marker([lat, lng]).addTo(leafletMap4);
...
leafletMap4.on('click', function(a) {console.log("lat, long: " + a.latlng.lat + "," + a.latlng.lng);});
